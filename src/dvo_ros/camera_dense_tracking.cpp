...
#include <dvo_core/surface_pyramid.h>
...
CameraDenseTracker::CameraDenseTracker(ros::NodeHandle& nh, ros::NodeHandle& nh_private) :
  ...
  vis_(new NoopCameraTrajectoryVisualizer()),
  use_dense_tracking_estimate_(false)
{
  ...
  tryGetTransform(from_baselink_to_asus, tl, "base_link", "asus");
...
void CameraDenseTracker::handleConfig(fast_slam::CameraDenseTrackerConfig& config, uint32_t level)
{
  if(level == 0) return;

  if(level & fast_slam::CameraDenseTracker_RunDenseTracking)
  {
  ...
  if(level & fast_slam::CameraDenseTracker_ConfigParam)
  {
    ...
    updateConfigFromDynamicReconfigure(config, tracker_cfg);
  ...
  if(level & fast_slam::CameraDenseTracker_MiscParam)
  {
    vis_->reset();
    delete vis_;
    ...
    if(config.reconstruction)
    {
      ...
      vis_ = new RosCameraTrajectoryVisualizer(nh_);
    }
    else
    {
      vis_ = new NoopCameraTrajectoryVisualizer();
