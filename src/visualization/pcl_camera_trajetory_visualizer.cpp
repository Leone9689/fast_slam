...
#include <visualization/pcl_camera_trajectory_visualizer.h>
...
class PclCameraVisualizer : public CameraVisualizer
{
public:
  ...
  virtual CameraVisualizer& update(const RgbdImage& img, const IntrinsicMatrix& intrinsics, const Eigen::Affine3d& pose)
  {
    ...
    cloud_.reset(new AsyncPointCloudBuilder::BuildJob(img, intrinsics, pose));
    return *this;
  }
...
private:
  Option visibility_;
  boost::shared_ptr<AsyncPointCloudBuilder::BuildJob> cloud_;
  boost::mutex m_;
};
...
CameraVisualizer::Ptr PclCameraTrajectoryVisualizer::camera(std::string name)
{
  return impl_->camera(name);
}

TrajectoryVisualizer::Ptr PclCameraTrajectoryVisualizer::trajectory(std::string name)
{
