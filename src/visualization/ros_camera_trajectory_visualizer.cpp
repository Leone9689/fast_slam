...
#include <visualization/ros_camera_trajectory_visualizer.h>
...
{
public:
  RosCameraVisualizer(std::string name, interactive_markers::InteractiveMarkerServer& marker_server, PointCloudAggregator& point_cloud_aggregator) :
  ...
  virtual CameraVisualizer& update(const RgbdImage& img, const IntrinsicMatrix& intrinsics, const Eigen::Affine3d& pose)
  {
    // update marker
...
private:
  ...
  PointCloudAggregator& point_cloud_aggregator_;
...
private:
  ...
  void update(const ros::TimerEvent& e)
  {
    ...
    AsyncPointCloudBuilder::PointCloud::Ptr cloud = point_cloud_aggregator_.build();
...
CameraVisualizer::Ptr RosCameraTrajectoryVisualizer::camera(std::string name)
{
  return impl_->camera(name);
}

TrajectoryVisualizer::Ptr RosCameraTrajectoryVisualizer::trajectory(std::string name)
{
