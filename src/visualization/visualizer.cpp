...
static inline void show1(const char* name, const cv::Mat& img)
{
  if(img.type() == CV_32F)
  {
    double min, max;
...
public:
  struct CreateHistogramFunc
  {
    ...
    void operator()()
    {
      ...
      compute1DHistogram(img, hist, min, max, binsize);
    ...
    void save(std::string filename)
    {
      ...
      compute1DHistogram(img, hist, min, max, 1);
  ...
  void visWorker()
  {
    ...
    while(!shutdown_)
    {
      if(image_queue_.pop_back(&named_img))
      {
        ...
        show1(named_img.name.c_str(), img);
